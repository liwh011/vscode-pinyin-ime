 ...
 * @returns 所有匹配结果
 */
const searchDB = (py: string, length: number): Promise<{ pinyin: string; word: string; }[]> => {
...
const splitPinyin = (py: string) => {
    const reg = /[^aoeiuv]?h?[iuv]?(ai|ei|ao|ou|er|ang?|eng?|ong|a|o|e|i|u|ng|n)?/g
...
const makeAllCandidates = async (py: string) => {
    ...
    for (let i = 0; i < splited.length; i++) {
        ...
        let subCandidates = Array.from(searchResult, (v) => {
            // 将未能匹配到的拼音串加入到对象中
            const suffix = splited.slice(splited.length - i).map(v => v.replace('%', '')).join('')
